<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="mainRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="顾客ID" prop="customerId">
          <el-input v-model="form.customerId" placeholder="请输入顾客ID" id="customerId" />
        </el-form-item>
        <el-form-item label="顾客姓名" prop="customerName">
        ...
        <el-form-item label="实际支付金额" prop="actualAmount">
          <el-input v-model="form.actualAmount" placeholder="请输入实际支付金额" id="actualAmount" />
        </el-form-item>
        <el-form-item label="优惠金额" prop="discountAmount">
          <el-input v-model="form.discountAmount" placeholder="请输入优惠金额" id="discountAmount" />
        </el-form-item>
        ...
        <el-form-item label="配送地址(外卖)" prop="deliveryAddress" v-show="form.orderType === 2">
          <el-input v-model="form.deliveryAddress" type="textarea" placeholder="请输入配送地址" id="deliveryAddress" />
        </el-form-item>
        ...
        <el-table :data="orderDetailList" :row-class-name="rowOrderDetailIndex" @selection-change="handleOrderDetailSelectionChange" ref="orderDetail">
          ...
          <el-table-column label="产品名称" prop="productName" width="180">
            <template #default="scope">
              <el-select v-model="scope.row.productName" placeholder="请选择产品名称" @change="handleProductChange(scope.row, $event)">
                <el-option
    ...
    <!-- 订单详情对话框 -->
    <el-dialog :title="'订单详情 - ' + detailForm.orderNo" v-model="detailOpen" width="800px" append-to-body>
        ...
        <div id="orderStatusFlow" style="margin-top: 30px;">
          ...
          <div class="flow-container">
            <template v-for="(item, index) in filteredStatusFlowList" :key="item.key">
              ...
              <div 
                v-if="index < filteredStatusFlowList.length - 1"
                class="flow-line"
...
/* 已完成节点 */
.flow-node.completed {
  background: #dff0d8; /* 浅绿色 */
  color: #3c763d;
}

/* 当前节点 */
.flow-node.active {
  background: #d9edf7; /* 浅蓝色 */
  color: #31708f;
